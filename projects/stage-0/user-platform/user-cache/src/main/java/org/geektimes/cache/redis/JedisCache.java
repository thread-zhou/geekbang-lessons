...
public class JedisCache<K extends Serializable, V extends Serializable> extends AbstractCache<K, V> {
    ...
    protected ExpireAbleEntry<K, V> getEntry(byte[] keyBytes) throws CacheException, ClassCastException {
        byte[] valueBytes = jedis.get(keyBytes);
        return ExpireAbleEntry.of(deserialize(keyBytes), deserialize(valueBytes));
    }
    ...
    protected ExpireAbleEntry<K, V> removeEntry(K key) throws CacheException, ClassCastException {
        byte[] keyBytes = serialize(key);
        ExpireAbleEntry<K, V> oldEntry = getEntry(keyBytes);
        jedis.del(keyBytes);
        return oldEntry;
    }
